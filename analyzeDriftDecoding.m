...
%     meanAcc(badRoi(iloc,:)==1,iloc) = 0.5;
    plot(1:numRois, meanAcc(:,iloc),'.','markersize',40,'color',squeeze(facecolorVec(iloc,:,:)));
...
for iloc=1:numLocs
    shift = (iloc-1.5)*0.2;
   temp = plot([ roiInd+shift; roiInd+shift], [minRandAcc(:,iloc)'; maxRandAcc(:,iloc)'], 'linewidth',3,'color',squeeze(facecolorVec(iloc,:,:)).^0.5);
   p(iloc) = temp(1);%for legend, keep plot properties of first ROI
   hold on
   plot([ roiInd+shift-0.3; roiInd+shift+0.3], [rand95(:,iloc)'; rand95(:,iloc)'], 'linewidth',3,'color',squeeze(facecolorVec(iloc,:,:)).^0.5);
   plot([ roiInd+shift-0.3; roiInd+shift+0.3], [rand05(:,iloc)'; rand05(:,iloc)'], 'linewidth',3,'color',squeeze(facecolorVec(iloc,:,:)).^0.5);
...
%    plot(roiX+(rand(size(roiX))-0.5)/6,subAcc(:,roiInd,iloc),'o','markersize',5,'color',plotColors{expNum});
   ...
   plot(roiInd(signifRoiInd)+shift, meanAcc(roiInd(signifRoiInd),iloc), '.','markersize',40,'color',plotColors{2});
