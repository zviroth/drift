...
locThresh = 0.1; %only include voxels with coherence>locThresh, for each of the two localizers.
...
locStr = thrsh([1 3:4]);
load([saveFolder 'decodeAccuracy_' expName '_' crossValidation '_thresh' locStr '.mat'],'crossValidation','locThresh','ROIs','subNames','dataFolder',...
...
facealpha = 0.3;
ilocTitle = {'stim','eye'};
randDist = squeeze(mean(randAccDist,1));
meanAcc = squeeze(mean(decodingAcc,1));
for iroi=1:numRois
    for iloc=1:2
%         decodingAcc(isub,iroi,iloc) = res{iloc}.accuracy;
%         randAccDist(isub,iroi,iloc,:) = res{iloc}.randAccuracy;
        
        temp = randAccDist(:,iroi,iloc,:);
...
for iloc=1:2
    for iroi=1:numRois
       if sum(isnan( decodingAcc(:,iroi,iloc)))>0
          %mark this ROI
          badRoi(iloc,iroi) = 1;
...
title([expName ', ' crossValidation ' cv, ' num2str(size(decodingAcc,1)) ' subjects']);
