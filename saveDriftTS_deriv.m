...
%                 subNames = {'s005120180524','s005720180906'};
...
for isub=1:numSubs
    ...
    for iroi=1:numRois
        ...
        if rois.n>0
            ...
            %add hdr derivative
            hdrderiv= [0 diff(hdr)];
            trialscmhdrderiv = conv2(trialscm,hdrderiv');
            trialscmhdrderiv = trialscmhdrderiv(1:size(rois.tSeries,2),:);
            
            trialscmhdrderiv = [trialscmhdr trialscmhdrderiv];
            ...
            driftBetasDeriv{isub,iroi} = getr2timecourse(rois.tSeries, size(trialscmhdrderiv,2), hdrlen, trialscmhdrderiv);
        else
            driftBetas{isub,iroi} = [];
            driftBetasDeriv{isub,iroi} = [];
        end
...
%%
varinfo=whos('driftBetasDeriv');
saveopt='';
if varinfo.bytes >= 2^31
    saveopt='-v7.3';
end
save([saveFolder 'driftTS_deriv_' expName '.mat'],'ROIs','subNames','dataFolder',...
    'stimLoc','eyeLoc','driftBetas','driftBetasDeriv','subRoiNans','subNumTrials','subNumScans',saveopt);
